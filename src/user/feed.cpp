...
bool feed::init( int tag, cinder::vec2 position )
{
    ...
    auto ciecle = add_child( renderer::circle::create( _radius, 5 ) );
    ciecle->set_color( hsvToRgb( vec3( randFloat( ), randFloat( 0.5, 0.8 ), randFloat( 0.5, 0.8 ) ) ) );
    ciecle->set_scale( vec2( 0.0F ) );
    ...
    ciecle->run_action( sequence::create( ease<EaseOutSine>::create( scale_to::create( 0.2F, vec2( target_scale ) ) ),
                                           repeat_forever::create( seq ) ) );

    ciecle->run_action( repeat_forever::create( rotate_by::create( 5.0F, randBool( ) ? M_PI * 2 : -M_PI * 2 ) ) );
